            ...
            <div class="nav_header">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" :inline="true" class="demo-form-inline">
                    <el-form-item label="供应商" prop="custID">
                        <el-select v-model="ruleForm.custID" filterable placeholder="请选择" size="mini">
                            <el-option
            ...
            <div>
                <el-table :data="replenishmentList" ref="multipleTable" tooltip-effect="dark"
                    ...
                    <el-table-column prop="inAmount" label="输入数量">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.inAmount" type="number"
                                      placeholder="请输入" min="1" @blur="numberChange(scope.$index,scope.row)" v-enter-number></el-input>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="maxPrice" label="选择单位单价（元）">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.maxPrice" type="number"
                                      placeholder="请输入" @change="priceChange(scope.$index,scope.row)"></el-input>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="totalPay" label="总金额（元）"></el-table-column>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    custID: [{required: true, message: '请选择入供应商', trigger: 'change'}],
        ...
        methods: {
                ...
                API.codeProductDetail(params)
                    .then(res => {
                        if (res.stateCode == 100) {
                            ...
                            that.replenishmentList[index].totalPay = (res.data.amount * res.data.POPrice).toFixed(2);
                        } else {
                            that.$message.error({
                                message: res.message,
                                duration: 2000
            ...
            numberChange: function (index, row) {
                ...
                that.replenishmentList[index].totalPay = totalPrice;
                // that.$set(that.replenishmentList[index], 'totalPay', amount)
            },
            // 默认单位改变
                ...
                this.$refs.ruleForm.validate(valid => {
                    if (valid) {
                        ...
                        var params = {
                            ...
                            purchaseDList: that.replenishmentList
                        }
                        API.addProcurement(params)
                            .then(res => {
                                if (res.stateCode == 100) {
                                    that.$message({
                                        message: res.message,
                                    ...
                                    if(num==1){
                                        ...
                                        that.$router.push({path:"/stockControl/purchase/purchase"})
                                    }
