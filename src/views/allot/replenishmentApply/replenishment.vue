                ...
                <el-table :data="tableData" style="width: 100%" highlight-current-row @current-change="detailClick">
                    ...
                    <el-table-column label="操作" width="150">
                        <template slot-scope="scope">
                            ...
                            <el-button type="primary" size="mini" @click.stop="delateClick(scope.row)" v-if="scope.row.isConfirm ==0" >删除 </el-button>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            handleDownload(){
                ...
                API.exportRP(params)
                    .then(res => {
                        ...
                        link.download = '调拨申请.xlsx'; //下载的文件名
            ...
            showAddDialog: function () {
                var that = this;
                that.$router.push("/allot/replenishmentApply/addReplenishment")
            },
            // 数据显示
            ...
            showEditDialog: function (row) {
                var that = this;
                that.$router.push({path: "/allot/replenishmentApply/editReplenishment", query: {detail: row}})
            },
            //详情
            showDetail: function (row) {
                var that = this;
                that.$router.push({path: "/allot/replenishmentApply/applyDetail", query: {detail: row}})
            },
            // 删除
            delateClick:function(row){
               var that = this;
