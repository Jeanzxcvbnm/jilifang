        ...
        <div class="pad_10 backfff martp_15">
            ...
            <div>
                <el-table :data="orderDetail" show-summary :summary-method="getSummaries" max-height="400"
                    ...
                    <el-table-column prop="inAmount" label="出库数量"></el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            handleDownload(){
                ...
                API.exportTO(params)
                    .then(res => {
                        ...
                        link.download = '调拨出库.xlsx'; //下载的文件名
            ...
            showEditDialog: function (row) {
                var that = this;
                that.$router.push({path: "/allot/allotDelivery/stayDelivergoods", query: {detail: row}})
            },
            // 确认发货
            affirmClick: function (row) {
                ...
                API.getStockOrdersDetails(outParams)
                    .then(res => {
                        if (res.stateCode == 100) {
                            var params = {
                                billID: row.billID,
