        ...
        <div class="pad_5 backfff">
            ...
            <div>
                <el-table :data="tableData" style="width: 100%" highlight-current-row @current-change="detailClick">
                    ...
                    <el-table-column prop="stockTotype" label="单据状态" :formatter="formatType"></el-table-column>
                    ...
                    <el-table-column label="操作" width="180">
                        <template slot-scope="scope">
                            ...
                            <el-button type="primary" size="mini" @click.stop="affirmClick(scope.row,3)" v-if="scope.row.isConfirm==1">确认审核</el-button>
                            <el-button type="primary" size="mini" @click.stop="affirmClick(scope.row,5)" v-if="scope.row.isConfirm==6">确认复核</el-button>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            handleDownload(){
                ...
                API.exportCS(params)
                    .then(res => {
                        ...
                        link.download = '采购审核.xlsx'; //下载的文件名
            ...
            showEditDialog: function (row) {
                var that = this;
                if (row.isConfirm == 1) {
                    that.$router.push({path: "/procurementBusiness/purchasingAudit/toAuditDetail", query: {detail: row}})
                }else if(row.isConfirm == 6){
                    that.$router.push({path: "/procurementBusiness/purchasingAudit/toReviewDetail", query: {detail: row}})
                } else {
                    that.$router.push({path: "/procurementBusiness/purchasingAudit/purchasingAuditDetail", query: {detail: row}})
                }
            },
            // 发送订单
            affirmClick:function(row,num){
                var that = this;
                if(num==3){
