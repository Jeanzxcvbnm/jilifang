            ...
            <div>
                <el-table :data="tableData" style="width: 100%" highlight-current-row @current-change="detailClick">
                    ...
                    <el-table-column label="操作" width="300">
                        <template slot-scope="scope">
                            <el-button type="primary" size="mini" @click="showEditDialog(scope.row)" v-if="scope.row.isConfirm==0 || scope.row.isConfirm==4" >编辑 </el-button>
                            <el-button type="primary" size="mini" @click="showDetail(scope.row)" v-if="scope.row.isConfirm!=0 && scope.row.isConfirm!=4">详情 </el-button>
                            <el-button type="primary" size="mini" @click.stop="sendClick(scope.row,1)" v-if="scope.row.isConfirm==0  || scope.row.isConfirm==4">发送订单 </el-button>
                            <el-button type="primary" size="mini" @click.stop="sendClick(scope.row,2)" v-if="scope.row.isConfirm==0  || scope.row.isConfirm==4">单据作废 </el-button>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            handleDownload(){
                ...
                API.exportOP(params)
                    .then(res => {
                        ...
                        link.download = '采购订货.xlsx'; //下载的文件名
            ...
            showAddDialog: function () {
                var that = this;
                that.$router.push("/procurementBusiness/purchasingOrder/addPurchasingOrder")
            },
            // 数据显示
            ...
            showEditDialog: function (row) {
                var that = this;
                that.$router.push({path: "/procurementBusiness/purchasingOrder/editPurchasingOrder", query: {detail: row}})
            },
            //详情
            showDetail: function (row) {
                var that = this;
                that.$router.push({path: "/procurementBusiness/purchasingOrder/purchasingOrderDetail", query: {detail: row}})
            },
            // 发送订单
            sendClick:function(row,num){
               var that = this;
                if(num==1){
                    this.$confirm('确认发送订单, 是否继续?', '提示', {
