            ...
            <div class="nav_header">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" :inline="true" class="demo-form-inline">
                    ...
                    <el-form-item label="发货仓库" prop="stockDest">
                        <el-select v-model="ruleForm.stockDest" filterable placeholder="请选择" @change="warehouseChange"
                                   size="mini">
                            <el-option
    ...
    export default {
        ...
        methods: {
            ...
            priceChange: function (index, row) {
                ...
                that.replenishmentList[index].price = (row.maxPrice / row.amount).toFixed(2);
                var totalPrice = (row.price * row.amount * row.disctRate / 100).toFixed(2);
                that.replenishmentList[index].totalPrice = totalPrice;
            },
            // 取消商品
            ...
            saveClick: function (num) {
                ...
                if (that.ruleForm.warehouseID == that.ruleForm.stockDest) {
                    that.$message.error({
                        message: "收货仓库、发货仓库不能重复",
                    });
                    return false;
                }
                            ...
                            .then(res => {
                                if (res.stateCode == 100) {
                                    ...
                                    if (num == 1) {
                                        ...
                                        that.$router.push({path: "/salesBusiness/salesReturn/salesReturn"})
                                    }
                                } else {
                                    that.$message.error({
                                        message: res.message,
