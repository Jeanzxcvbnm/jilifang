                ...
                <el-table :data="tableData" style="width: 100%" highlight-current-row @current-change="detailClick">
                    ...
                    <el-table-column label="操作"  width="230">
                        <template slot-scope="scope">
                            ...
                            <el-button type="primary" size="mini" @click="showDetail(scope.row)"  v-if="scope.row.isConfirm !=0">详情 </el-button>
                            <el-button type="primary" size="mini" @click.stop="sendClick(scope.row)" v-if="scope.row.isConfirm ==0">确认发送</el-button>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            handleDownload(){
                ...
                API.exportSI(params)
                    .then(res => {
                        ...
                        link.download = '销售退货.xlsx'; //下载的文件名
            ...
            showAddDialog: function () {
                var that = this;
                that.$router.push("/salesBusiness/salesReturn/addSalesReturn")
            },
            // 数据显示
            ...
            showEditDialog: function (row) {
                var that = this;
                that.$router.push({path: "/salesBusiness/salesReturn/editSalesReturn", query: {detail: row}})
            },
            //详情
            showDetail: function (row) {
                var that = this;
                that.$router.push({path: "/salesBusiness/salesReturn/salesReturnDetail", query: {detail: row}})
            },
            // 发送订单
