                    ...
                    <el-table-column label="操作" width="280">
                        <template slot-scope="scope">
                            <el-button type="primary" size="mini" @click="showEditDialog(scope.row)"
                                       v-if="scope.row.isConfirm ==0">编辑
                            </el-button>
                            <el-button type="primary" size="mini" @click="showDetail(scope.row)"
                                       v-if="scope.row.isConfirm !=0">详情
                            </el-button>
                            <el-button type="primary" size="mini" @click.stop="sendClick(scope.row,1)"
                                       v-if="scope.row.isConfirm ==0">发送订单
                            </el-button>
                            <el-button type="primary" size="mini" @click.stop="sendClick(scope.row,2)"
                                       v-if="scope.row.isConfirm ==0">单据作废
                            </el-button>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            handleDownload(){
                ...
                API.exportOS(params)
                    .then(res => {
                        ...
                        link.download = '销售订货.xlsx'; //下载的文件名
            ...
            showAddDialog: function () {
                var that = this;
                that.$router.push("/salesBusiness/salesQuotation/addSalesQuotation")
            },
            // 数据显示
            ...
            showEditDialog: function (row) {
                var that = this;
                that.$router.push({path: "/salesBusiness/salesQuotation/editSalesQuotation", query: {detail: row}})
            },
            //详情
            showDetail: function (row) {
                var that = this;
                that.$router.push({path: "/salesBusiness/salesQuotation/salesQuotationDetail", query: {detail: row}})
            },
            // 发送订单 订单作废
            // 发送订单
